...
// =========================================================
  ...
  const {viscosite}=grandeurs;
  const {nom,couleur,categorie}=identification;const {codor}=couleur;
  const {min,max,moy}=nombers(viscosite.normes);
...
//   const { MA ,setMA}=useMA(ma0);
  const [ob,setOb]=useState({machine:machine,nom:nom,reservoir:reservoir,visco:value,categorie:categorie});
  function APPRECIATIONS(){
    const lVisco=value<min?'Visco Low':"";
    const hVisco=value>max?'Visco High':"";
    const appreciations=lVisco+hVisco;
    return appreciations===""?"RAS":appreciations;
  }

   function handleJoin(vl){
    const isMera=nom.split(" ").includes("Mera");
    const maxLength=isMera?3:4;
    if(vl==="DEL"){
    ...
    if(typeof(vl)==="number" && String(value).length<maxLength){
  ...
  return <View style={[ styles.container1,{marginBottom:50,backgroundColor:"transparent",}]}>
    <View style={styles.viscometreStyles}>
    <View style={styles.viscometreMainScreen}>
      <View style={styles.viscometreScreen}>
          <Text style={{maxHeight:20,minHeight:20,fontSize:10,alignContent:"center",minWidth:'100%',backgroundColor:"transparent"}}>
            {((value<min) && <Text style={{width:'30%',}}>❌Viscosité Low</Text>) ||((value>max) && <Text style={{width:'30%',}}>❌Viscosite High</Text>)}
            </Text>
          <Slider
...
const styles = StyleSheet.create({...styleRange,viscometreStyles:{
    // width:"100%",
    maxWidth:400,
    minWidth:400,
