...
export const ProductsProvider=({children})=>{
    ...
    const [meraResults,setMeraResults]=useState(null); 
   ...
   useLayoutEffect(()=>{// les asynchrones il faut qu'elles passent par le hokk useEffect || useLayoutEffect car elles sont en retard en synchrone
         async function fetchCosmetiquesResults() {
           ...
           if (co_results) {setSplitResults(co_results);}
         }
         async function fetchData() {
             const results = await getResults();
             if (results) {setResults(results);}
         }
         async function fetchMera() {
             const mera_results = await getMeraResults();
             if (mera_results) {setMeraResults(mera_results);}
         }
         fetchData();
