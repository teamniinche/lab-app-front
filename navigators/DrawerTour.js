...
const PowderRow=({K,ky,ac,setK,item})=>{
    ...
    const {setPop}=usePopup();
    const {setId2Update,id2Update,setAction,setFocusedList,focusedPro,ListOfFocusedAndLastNumberTour,keysAndRequirements,registred,setRegistred}=useCurrentProducted();
    const {noErrors,itemToSave,setHandleUpdateT}=useItemToSave();
            ...
            >
                ...
                >
                    ...
                    {(id2Update!==null && isMeToUpdate)?
                        <EditRowUpdate /*render={(vl)=>handleUpdateTour(vl)}*/ inputRefs={[]} index={0} />
                        :
...
const EditRowUpdate=({keyOk,inputRefs,index})=>{ // import de DrawerPoudre
...
const LansaValue=({inputRefs,labelJoined,renderSave,keyOk,index,icon,value,renderValidity,renderVal})=>{ // import de DrawerPoudre
    ...
    const {itemToSave,setItemToSave,setIpt,setIndex,INDEX}=useItemToSave();
...
const Actions=({item})=>{// import de DrawerPoudre
  ...
  return <TouchableWithoutFeedback>
    ...
    {
      (id2Update && id2Update===item?.id)?
      <><Pressable style={{...style.actions,marginRight:45,}} onPress={handleCancel}>
            <Icon size={14} name="times" color={couleurs[delKey][8]}/>
        </Pressable>
        <Pressable style={{...style.actions,marginLeft:50,}} disabled={!updateAllowed} onPress={handleCheckValidity}>
            <Icon size={14} name="save" color={couleurs[updKey][7]}/>
        </Pressable>
        </>:
      <>
