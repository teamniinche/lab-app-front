...
import { isFormule,allowTo} from '../assets/functions';
...
const AnalysedList=() => {
        ...
        useMemo(()=>{setAnalysed(focusedListe)},[focusedListe]);// focusedListe du store etait import pour cette partie
...
export const AnalysedListe=({product,rend}) => {// Pour Poudre-full
        ...
        // }
        useLayoutEffect(()=>{
            fetch(dbBaseRoot+"poudre/analyses/")
            .then(response=>response.json())
            .then(data=>setAnalysed(data.analyses))
            .catch(function(error){
                alert(error.message)
            })
        ...
        useMemo(()=>{
            setAnalysed(powderType),
            rend(powderType)},[powderType]);// focusedListe du store etait import pour cette partie
        const AnalysedLen=analysed.length;
        const {GG,HUMIDITE,MATIERE_ACTIVE,ALCANITE}=Everages(analysed);
...
const Chariot=({oldCh})=>{
    ...
    const {powderAnalysed,token}=useSelector(state=>{
        const powderAnalysed=state.powderAnalysed.powderAnalysed;
        const token=state.user.targetUser.token;
    return {powderAnalysed:powderAnalysed,token:token}}
    )
    function handleBlur(newCh){
        ...
        if(!item.id){alert("Erreur sur l'ID cible ! Peut-etre UNDEFINED | NULL .");return}
...
const PoudreWorkSpace=() => {
    ...
    const {toCreate,setAction,ListOfFocusedAndLastNumber,currentProductedPro,currentProductedLen,setFocusedList,setFocusedPro,focusedPro,keysAndRequirements,registred,setRegistred}=useCurrentProducted();
        ...
        if(!noErrors){
            ...
            if(missings.length!==0){
                ...
                if(toCreate){
                    ...
                    const buildItem={...rest,...itemToSave,identifier:identifier,nChar:parseFloat(freeChariot),observations:observations,categorie:"local",UtilisateurId:1};
                    ...
                    const builtItem={...rest,...itemToSave,observations:observations,categorie:"local",UtilisateurId:1};
                    const enregistre=[...registredWithoutTheFocused,builtItem];// 2
                    // ===================== Envoi aux memoires Context & store =====================
                    setFocusedList([...list,{...rest,...itemToSave,observations:observations,UtilisateurId:1}].sort((firstItem, secondItem) => Number(firstItem.nChar )- Number(secondItem.nChar)));
